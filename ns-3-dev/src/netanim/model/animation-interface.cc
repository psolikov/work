    ...
    netanim(0)
{
  ...
  application->setApplicationName ("NetAnimOnline");
  netanim = new netanim::NetAnim (true);
  animxmlparser = netanim::AnimatorMode::getInstance ()->getAnimxmlparser (); 
  // ShowApplication();
  StartAnimation ();
...
void
AnimationInterface::ShowApplication ()
{
  static char name[] = "NetAnimOnline";
  char *argv = name;
  static int argc = 1;
  application = new QApplication (argc, &argv);
  application->setApplicationName ("NetAnimOnline");
...
AnimationInterface::EnableIpv4RouteTracking (std::string fileName, Time startTime, Time stopTime, Time pollInterval)
{
  ...
  {
    if(animxmlparser){
      // animxmlparser->doParse(WriteAnim ());
    }
  }
...
AnimationInterface::AddNodeCounter (std::string counterName, CounterType counterType)
{
  ...
  else
  {
    if(animxmlparser)
    {
      // animxmlparser->doParse(WriteAddNodeCounter (counterId, counterName, counterType));
    }
  }
  return counterId; 
}
...
AnimationInterface::AddResource (std::string resourcePath)
{
  ...
  else
  {
    if(animxmlparser)
    {
      // animxmlparser->doParse(WriteAddResource (resourceId, resourcePath));
    }
  }
  return resourceId; 
}
...
AnimationInterface::UpdateNodeImage (uint32_t nodeId, uint32_t resourceId)
{
  ...
  else
  {
    if(animxmlparser)
    {
      // animxmlparser->doParse(WriteUpdateNodeImage (nodeId, resourceId));
    }
  }
}
...
AnimationInterface::UpdateNodeCounter (uint32_t nodeCounterId, uint32_t nodeId, double counter)
{
  ...
  else
  {
    if(animxmlparser)
    {
      // animxmlparser->doParse(WriteUpdateNodeCounter (nodeCounterId, nodeId, counter));
    }
  }
}
...
AnimationInterface::SetBackgroundImage (std::string fileName, double x, double y, double scaleX, double scaleY, double opacity)
{
  ...
  else
  {
    if(animxmlparser)
    {
      // animxmlparser->doParse(WriteUpdateBackground (fileName, x, y, scaleX, scaleY, opacity));
    }
  }
}
...
AnimationInterface::UpdateNodeSize (uint32_t nodeId, double width, double height)
{
  ...
  else
  {
    if(animxmlparser)
    {
      // animxmlparser->doParse(WriteUpdateNodeSize (nodeId, s.width, s.height));
    }
  }
}
...
AnimationInterface::UpdateNodeColor (uint32_t nodeId, uint8_t r, uint8_t g, uint8_t b)
{
  ...
  else
  {
    if(animxmlparser)
    {
      // animxmlparser->doParse(WriteUpdateNodeColor (nodeId, r, g, b));
    }
  }
}
                                                ...
                                                std::string linkDescription)
{
  ...
  else
  {
    if(animxmlparser)
    {
      // animxmlparser->doParse(WriteUpdateLink (fromNode, toNode, linkDescription));
    }
  }
}
                                                ...
                                                std::string linkDescription)
{
  ...
  else
  {
    if(animxmlparser)
    {
      // animxmlparser->doParse(WriteUpdateLink (fromNode->GetId (), toNode->GetId (), linkDescription));
    }
  }
}
...
AnimationInterface::UpdateNodeDescription (uint32_t nodeId, std::string descr)
{
  ...
  else
  {
    if(animxmlparser)
    {
      // animxmlparser->doParse(WriteUpdateNodeDescription (nodeId));
    }
  }
}
...
AnimationInterface::MobilityCourseChangeTrace (Ptr <const MobilityModel> mobility)
{
  ...
  else
  {
    if(animxmlparser)
    {
      // animxmlparser->doParse(WriteUpdateNodePosition (n->GetId (), v.x, v.y));
    }
  }
}
...
AnimationInterface::WriteNonP2pLinkProperties (uint32_t id, std::string ipv4Address, std::string channelType)
{
  ...
  else
  {
    if(animxmlparser)
    {
      // animxmlparser->doParse(WriteNonP2pLinkProperties (id, ipv4Address));
    }
  }
}
...
AnimationInterface::OutputWirelessPacketTxInfo (Ptr<const Packet> p, AnimPacketInfo &pktInfo, uint64_t animUid)
{
  ...
  else
  {
    if(animxmlparser)
    {
      // animxmlparser->doParse(WritePRef (animUid, nodeId, pktInfo.m_fbTx, m_enablePacketMetadata? GetPacketMetadata (p):""));
    }
  }
}
...
AnimationInterface::OutputWirelessPacketRxInfo (Ptr<const Packet> p, AnimPacketInfo & pktInfo, uint64_t animUid)
{
  ...
  else
  {
    if(animxmlparser)
    {
      // animxmlparser->doParse(WriteP (animUid, "wpr", rxId, pktInfo.m_fbRx, pktInfo.m_lbRx));
    }
  }
}
...
AnimationInterface::OutputCsmaPacket (Ptr<const Packet> p, AnimPacketInfo &pktInfo)
{
  ...
  else
  {
    if(animxmlparser)
    {
      /*animxmlparser->doParse(WriteP ("p", 
               nodeId, 
               pktInfo.m_fbTx, 
               pktInfo.m_lbTx, 
               rxId,
       ...
       ++i)
    {
      ...
      for (NodeIdIpv4Map::const_iterator it = iterPair.first;
        ...
        else
        {
          if(animxmlparser)
          {
            // animxmlparser->doParse(WriteIpv4Addresses (i->first, ipv4Addresses));
          }
        }
    }
}
       ...
       i = m_nodeIdIpv6Map.upper_bound (i->first))
    {
      ...
      for (NodeIdIpv6Map::const_iterator it = iterPair.first;
        ...
        else
        {
          if(animxmlparser)
          {
            // animxmlparser->doParse(WriteIpv6Addresses (i->first, ipv6Addresses));
          }
        }
    }
}
              ...
              for (uint32_t j = 0; j < nChDev; ++j)
                {
                  ...
                  if (n1Id < n2Id)
                    { 
                      ...
                      {
                        if(animxmlparser){
                          // animxmlparser->doParse(WriteLink(n1Id, 0, n2Id));
                        }
                      }
                    }
                }
  ...
  for (NodeList::Iterator i = NodeList::Begin (); i != NodeList::End (); ++i)
    {
      ...
      {
        if(animxmlparser){
          // animxmlparser->doParse(WriteNode(n->GetId (), n->GetSystemId (), v.x, v.y));
        }
      }
    }
}
...
AnimationInterface::WriteAnim (bool routing)
{
  ...
  /*if(animxmlparser){
    animxmlparser->doParse(parsedElement);
  }*/
