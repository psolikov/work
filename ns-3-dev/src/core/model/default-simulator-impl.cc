...
bool DefaultSimulatorImpl::isOnline = false;
uint64_t DefaultSimulatorImpl::nextTs = -1;
...
DefaultSimulatorImpl::Run (void)
{
  ...
  else
  {
    ...
    /*netanim::AnimatorMode * animMode = netanim::AnimatorMode::getInstance ();
    if (!isRunning)
    {
      isRunning = true;
      ProcessElementsWithSameTime ();
      animMode->parseOnline ();
      QApplication::exec ();
    }
