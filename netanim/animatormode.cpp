...
#include "/home/pavel/repos2/ns-3-allinone/ns-3-dev/src/core/model/default-simulator-impl.h"
...
{
  animxmlparser = new Animxmlparser ("");
  init ();
}
...
bool
AnimatorMode::parseOnline()
{
  m_events.add (std::numeric_limits<qreal>::max (), new AnimNIL ());
  // showParsingXmlDialog (true);
  // m_rxCount = parser.getRxCount ();
  m_rxCount = 0; //??
  setProgressBarRange (m_rxCount);
  // parser.doParse ();
  m_lastPacketEventTime = animxmlparser->getLastPacketEventTime ();
  m_thousandthPacketTime = animxmlparser->getThousandthPacketTime ();
  m_firstPacketEventTime = animxmlparser->getFirstPacketTime ();
  m_minPoint = animxmlparser->getMinPoint ();
  m_maxPoint = animxmlparser->getMaxPoint ();
  // showParsingXmlDialog (false);
  // setMaxSimulationTime (animxmlparser->getMaxSimulationTime ());
  ...
  if (result == m_events.GOOD)
    {
      if (OnlineMode) 
      {
        if (pp.first->first >= (qreal) ns3::DefaultSimulatorImpl::getNextTs () / 1e9)
            {
              nextTs = pp.first->first;
              return;
            }
      }
...
void
AnimatorMode::setOnlineMode(bool mode)
{
  OnlineMode = mode;
}

// void
// AnimatorMode::setEventsOver (bool isOver)
// {
//   eventsOver = isOver;
// }

bool
AnimatorMode::getOnlineMode()
{
  return OnlineMode;
}
